...
--   x (FORMAT 'ZZZ,...')   -> dropped (display formatting only)
--   CURRENT_FLAG='Y' / IS_ACTIVE SCD filters -> collapse to IS_ACTIVE=1 in the
--                             single-version natural-key parity model.
--   90-day CURRENT_DATE txn window -> replaced by deterministic LIFETIME
--                             aggregates. The source's TXN_COUNT_90D /
--                             TXN_AMOUNT_90D depend on CURRENT_DATE (evaluated in
--                             2026 they would return zero over 2024-2025 seeds),
--                             so the parity signature uses lifetime_txn_count /
--                             lifetime_txn_amount over BASE_CURRENCY_AMOUNT.
