...
--   a || ' ' || b           -> supported in GoogleSQL (equivalently CONCAT)
--   FORMAT 'ZZZ,...'        -> dropped; format in the presentation layer
--   The parity model uses lifetime transaction aggregates (no CURRENT_DATE / 90-
--   day window) so the signature metrics are deterministic across runs.
