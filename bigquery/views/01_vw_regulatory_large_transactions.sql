...
--   SEL                  -> SELECT
--   LOCKING ROW FOR ...  -> removed (no equivalent; BigQuery has no read locks)
--   NOT CASESPECIFIC     -> dropped; compare with LOWER()/UPPER() if needed
--   QUALIFY ROW_NUMBER() -> QUALIFY (supported natively in GoogleSQL)
--   HASHROW(...)         -> FARM_FINGERPRINT(...) in production; omitted here
--                           because it is not part of the parity contract.
