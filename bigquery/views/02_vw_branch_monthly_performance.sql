...
--   FORMAT 'ZZZ,...'        -> dropped; presentation formatting belongs to the
--                              consuming layer, not the warehouse view
--   ADD_MONTHS(d, n)        -> DATE_ADD(d, INTERVAL n MONTH) where a date filter
--                              is required; the deterministic parity model keeps
--                              the full seed history so no CURRENT_DATE filter is
--                              applied here
--   CSUM(x, sortkey)        -> SUM(x) OVER (ORDER BY sortkey
--                                 ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW)
--   MAVG(x, n, sortkey)     -> AVG(x) OVER (ORDER BY sortkey
--                                 ROWS BETWEEN (n-1) PRECEDING AND CURRENT ROW)
--                              MAVG(x, 3, ...) is a 3-row window: current row + 2
--                              preceding rows. Translating to "3 PRECEDING"
--                              averages 4 rows -- the off-by-one the parity
--                              harness flags on sum_moving_avg_volume.
